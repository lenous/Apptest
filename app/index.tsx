...
export default function ProductionApp() {
  ...
  useEffect(() => {
    loadPreviewDb().then((loaded) => {
      setDb(loaded);
      setScreen(defaultScreenForRole(currentUser(loaded).role) as Screen);
      setSelectedStationId(currentUser(loaded).defaultStation ?? 3);
      setLoading(false);
    });
  }, []);
  ...
  function switchRole(role: Role) {
    const nextUser = db.users.find((item) => item.role === role) ?? db.users[0];
    updateDb((draft) => {
      draft.sessionUserId = nextUser.id;
    });
    setSelectedUserId(nextUser.id);
    setSelectedStationId(nextUser.defaultStation ?? 3);
    setScreen(defaultScreenForRole(role) as Screen);
  }
  ...
  return (
    <SafeAreaView style={[styles.safe, db.darkMode && styles.safeDark]}>
      <View style={styles.header}>
        <View>
          <Text style={styles.headerKicker}>Aplikace pro výrobu</Text>
      ...
      {screen !== 'login' && (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.roleBar} contentContainerStyle={styles.roleBarContent}>
          {(Object.keys(ROLE_LABELS) as Role[]).map((role) => (
            <Chip key={role} label={ROLE_LABELS[role]} selected={user.role === role} onPress={() => switchRole(role)} />
          ))}
        </ScrollView>
      )}
      ...
      <View style={styles.bottomNav}>
        {navItems.map(([target, icon, label]) => (
          <TouchableOpacity key={target} style={styles.navItem} onPress={() => setScreen(target as Screen)}>
            <Ionicons name={`${icon}${screen === target ? '' : '-outline'}` as any} size={21} color={screen === target ? '#1a56db' : '#6b7280'} />
    ...
    return (
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.faceCard}>
          <Ionicons name="scan-circle-outline" size={92} color="#60a5fa" />
          <Text style={styles.faceTitle}>Rychlý vstup</Text>
          <Text style={styles.faceText}>Vyber roli nebo pracovníka pro demo provozu. Data se ukládají lokálně v zařízení.</Text>
        </View>
        ...
        <Section title="Uživatelé">
          {db.users.map((item) => (
            <TouchableOpacity
              ...
              onPress={() => {
                updateDb((draft) => {
                  draft.sessionUserId = item.id;
                });
                setSelectedStationId(item.defaultStation ?? 3);
                setScreen(defaultScreenForRole(item.role) as Screen);
              }}
            >
        ...
        >
          <Text style={styles.outlineButtonText}>Reset dat prototypu</Text>
        </TouchableOpacity>
      </ScrollView>
    );
  }
    ...
    return (
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.statsRow}>
          ...
          <Stat label="Probíhá" value={active} color="#1d4ed8" />
    ...
    return (
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.rowBetween}>
          ...
          {canCreateOrder(user.role) && <IconButton icon="add" onPress={() => setScreen('newOrder')} />}
        </View>
  ...
  function renderDashboard() {
    ...
    return (
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.statsRow}>
          <TouchableOpacity style={styles.statBox} onPress={() => setDashboardFilter('active')}><Stat label="Aktivní" value={inProgress} color="#1d4ed8" /></TouchableOpacity>
    ...
    return (
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.statsRow}>
          ...
          <Stat label="Kusy" value={totalQty} color="#1d4ed8" />
        </View>
        <View style={styles.statsRow}>
  ...
  function renderOrderCard(order: Order, onPress: () => void, stationId?: number) {
    ...
    const border = hasIssue ? '#b91c1c' : deadline === 'overdue' ? '#d97706' : deadline === 'soon' ? '#f59e0b' : '#1d4ed8';
        ...
        <View style={styles.modalShade}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.qtyPicker}>
              ...
              <Text style={styles.qtyNumber}>{transferModal?.qty ?? 0}</Text>
...
function IconButton({ icon, onPress, danger }: { icon: string; onPress: () => void; danger?: boolean }) {
  return (
    <TouchableOpacity style={[styles.iconButton, danger && styles.iconButtonDanger]} onPress={onPress}>
      <Ionicons name={icon as any} size={18} color={danger ? '#b91c1c' : '#1a56db'} />
    </TouchableOpacity>
  );
}
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: '#1a56db', paddingHorizontal: 18, paddingTop: 12, paddingBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  headerKicker: { color: '#bfdbfe', fontSize: 12, fontWeight: '700', textTransform: 'uppercase' },
  headerTitle: { color: '#fff', fontSize: 20, fontWeight: '800' },
  headerSub: { color: '#dbeafe', fontSize: 13, marginTop: 2 },
  headerButton: { width: 42, height: 42, alignItems: 'center', justifyContent: 'center' },
  roleBar: { maxHeight: 54, backgroundColor: '#1a56db' },
  roleBarContent: { paddingHorizontal: 14, paddingBottom: 12, gap: 8 },
  content: { flex: 1 },
  scroll: { padding: 14, paddingBottom: 104 },
  bottomNav: { flexDirection: 'row', borderTopWidth: 1, borderTopColor: '#e5e7eb', backgroundColor: '#fff', paddingVertical: 6 },
  navItem: { flex: 1, alignItems: 'center', gap: 2 },
  navText: { fontSize: 10, color: '#6b7280', fontWeight: '600' },
  navTextActive: { color: '#1a56db' },
  screenTitle: { fontSize: 21, fontWeight: '800', color: '#111827' },
  section: { marginTop: 14 },
  sectionTitle: { fontSize: 13, fontWeight: '800', color: '#475569', marginBottom: 8, textTransform: 'uppercase' },
  card: { backgroundColor: '#fff', borderRadius: 10, padding: 14, marginBottom: 10, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4, shadowOffset: { width: 0, height: 2 }, elevation: 1 },
  cardActive: { borderWidth: 1.5, borderColor: '#1a56db' },
  cardUnread: { backgroundColor: '#eff6ff' },
  cardDanger: { borderLeftWidth: 4, borderLeftColor: '#b91c1c' },
  cardTitle: { fontSize: 15, fontWeight: '800', color: '#111827' },
  bodyText: { color: '#334155', fontSize: 14, marginTop: 5, lineHeight: 20 },
  muted: { color: '#64748b', fontSize: 12, marginTop: 3 },
  tiny: { color: '#64748b', fontSize: 11 },
  errorText: { color: '#b91c1c', fontSize: 12, fontWeight: '700', marginTop: 8 },
  infoBox: { backgroundColor: '#dbeafe', borderRadius: 10, padding: 12, marginBottom: 10 },
  infoText: { color: '#1d4ed8', fontWeight: '700' },
  ...
  progress: { height: 7, borderRadius: 4, backgroundColor: '#e5e7eb', overflow: 'hidden', marginTop: 10, marginBottom: 7 },
  progressFill: { height: '100%', backgroundColor: '#1a56db' },
  stationRow: { backgroundColor: '#fff', borderRadius: 10, padding: 12, marginBottom: 8, borderLeftWidth: 4 },
  countGrid: { flexDirection: 'row', gap: 8 },
  countBox: { flex: 1, backgroundColor: '#fff', borderRadius: 10, padding: 10, alignItems: 'center' },
  countValue: { fontSize: 24, fontWeight: '900' },
  countLabel: { fontSize: 11, color: '#64748b', fontWeight: '800', textTransform: 'uppercase' },
  countControls: { flexDirection: 'row', gap: 8, marginTop: 9 },
  countButton: { width: 34, height: 30, borderRadius: 8, backgroundColor: '#eef2f7', alignItems: 'center', justifyContent: 'center' },
  countButtonText: { fontSize: 18, fontWeight: '900', color: '#111827' },
  checkRow: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#fff', padding: 12, borderRadius: 9, marginBottom: 7 },
  checkText: { flex: 1, color: '#334155', fontSize: 14 },
  checkTextDone: { color: '#15803d', textDecorationLine: 'line-through' },
  actionStack: { gap: 9, marginTop: 12 },
  primaryButton: { backgroundColor: '#1a56db', borderRadius: 10, paddingVertical: 12, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, marginTop: 8 },
  ...
  outlineButton: { borderWidth: 1.5, borderColor: '#1a56db', borderRadius: 10, padding: 12, alignItems: 'center', marginTop: 12 },
  outlineButtonText: { color: '#1a56db', fontWeight: '800' },
  outlineButtonFlex: { flex: 1, borderWidth: 1.5, borderColor: '#1a56db', borderRadius: 10, padding: 12, alignItems: 'center' },
  primaryButtonFlex: { flex: 1, backgroundColor: '#1a56db', borderRadius: 10, padding: 12, alignItems: 'center' },
  dangerButtonFlex: { flex: 1, backgroundColor: '#b91c1c', borderRadius: 10, padding: 12, alignItems: 'center' },
  iconButton: { width: 34, height: 34, borderRadius: 9, backgroundColor: '#eff6ff', alignItems: 'center', justifyContent: 'center' },
  iconButtonDanger: { backgroundColor: '#fee2e2' },
  input: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 10, minHeight: 43, paddingHorizontal: 12, fontSize: 15, color: '#111827' },
  textArea: { minHeight: 76, paddingTop: 10, textAlignVertical: 'top' },
  formField: { marginTop: 12 },
  label: { color: '#475569', fontSize: 12, fontWeight: '800', marginBottom: 5 },
  tabRow: { flexDirection: 'row', backgroundColor: '#e2e8f0', borderRadius: 11, padding: 4, marginTop: 12 },
  tab: { flex: 1, paddingVertical: 8, alignItems: 'center', borderRadius: 8 },
  tabActive: { backgroundColor: '#fff' },
  tabText: { fontSize: 12, color: '#475569', fontWeight: '800' },
  tabTextActive: { color: '#1a56db' },
  docRow: { flexDirection: 'row', alignItems: 'center', gap: 9, backgroundColor: '#fff', borderRadius: 9, padding: 12, marginBottom: 7 },
  docText: { flex: 1, color: '#334155', fontWeight: '700' },
  noteCard: { backgroundColor: '#fff', borderRadius: 9, padding: 11, marginTop: 8 },
  backRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 8 },
  backButton: { width: 38, height: 38, borderRadius: 10, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' },
  empty: { alignItems: 'center', justifyContent: 'center', padding: 24, backgroundColor: '#fff', borderRadius: 10 },
  link: { color: '#1a56db', fontWeight: '800' },
  dangerLink: { color: '#b91c1c', fontWeight: '800' },
  toggleRow: { backgroundColor: '#fff', borderRadius: 10, padding: 12, marginBottom: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  editorBlock: { marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#e2e8f0' },
  ...
  modalShade: { flex: 1, backgroundColor: 'rgba(15,23,42,0.55)', justifyContent: 'flex-end' },
  ...
  roundButton: { width: 46, height: 46, borderRadius: 23, backgroundColor: '#eef2f7', alignItems: 'center', justifyContent: 'center' },
  roundButtonText: { fontSize: 24, fontWeight: '900', color: '#111827' },
  qtyNumber: { fontSize: 36, fontWeight: '900', color: '#111827', minWidth: 96, textAlign: 'center' },
  faceCard: { backgroundColor: '#0f172a', borderRadius: 14, padding: 24, alignItems: 'center', marginBottom: 12 },
